...
def make_positions(tensor, padding_idx, left_pad):
    ...
    Replace non-padding symbols with their position numbers.
    Position numbers begin at padding_idx + 1.
    Padding symbols are left unchanged.
    """
    B, T = tensor.size()
    max_pos = padding_idx + 1 + T

    # 缓冲区：只在首次或尺寸不够时生成
    if not hasattr(make_positions, 'range_buf') or make_positions.range_buf.numel() < max_pos:
        make_positions.range_buf = torch.arange(
            padding_idx + 1, max_pos, dtype=torch.long, device=tensor.device
        )

    positions = make_positions.range_buf[:T].expand(B, T)

    mask = tensor.ne(padding_idx).to(positions.device)

    if left_pad:
        positions = positions - T + mask.long().sum(dim=1, keepdim=True)

    output = tensor.clone().to(positions.device).long()  # 🔧 确保 dtype 匹配
    output[mask] = positions[mask]
